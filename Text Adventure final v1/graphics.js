...
function resizeConsole() {
	...
	if(container.value == "maximize") {
		console.log("minimize");
		container.style.height = "30pt";
		container.value = "minimize";
	}
	else {
		console.log("minimize");
		container.style.height = "60%";
		container.value = "maximize";
	}
...
function saveMonster() {
	...
	monster.examine = document.getElementById("monster-examine").value;
	monster.hp = document.getElementById("monster-hp").value;
	monster.dp = document.getElementById("monster-dp").value;
...
function areaAddMonster() {
	global_areaArray[document.getElementById("area").value].monsters.push(global_monsterArray[document.getElementById("area-monster-select").value]);
	loadAreaPanel(document.getElementById("area").value);
}
function areaRemoveMonster() {
	global_areaArray[document.getElementById("area").value].monsters.splice(parseInt(document.getElementById("area-item-select").value), 1);
	loadAreaPanel(document.getElementById("area").value);
}
...
function loadAreaPanel(index) {
	...
	if(index == -1) {											//if the index equals -1 create a new Area (the new area option has a value of -1)
		...
		element.addEventListener("mouseenter", function () {overArea=this.id;})		//add panning code listeners -- used to distinguish between panning 'pan' and the area rep
	...
	var monsterText = document.getElementById("area-monsters-text");	//Same as destinations except for monsters
	monsterText.innerText = ""
	for(var i=0;i<area.monsters.length;i++) {
		monsterText.innerText = monsterText.innerText + "|" + area.monsters[i].name;
	}
	if(area.monsters.length > 0) {
		monsterText.innerText = monsterText.innerText + "|";
	}
...
function loadDests() {
	...
	newAreaOption.innerText="Add Area";
...
function loadItems() {
	...
	newItemOption.innerText="Add Item";
...
function loadMonsters() {
	...
	newMonsterOption.innerText="Add Monster";
...
function loadProject() {
	...
	var count = pan.childElementCount;
	for(var i=0; i<count;i++) {
		pan.removeChild(pan.firstElementChild);
	}
	console.log(pan);
	...
	for (var i=0; i < file.length; i++) {			//iterate through lines, taking appropriate action depending on different circumstances
		...
		if(loadedObject.class == "area") {			//if loadObject is an area add to global_areaArray
			...
			element.style.left = -pan.offsetLeft + 100;				//place area at an offset of 100 from left of viewport
			element.style.top = -pan.offsetTop + 100;				// place area at an offset of 100 from top of viewport
			element.addEventListener("mouseenter", function () {overArea=this.id;})	//add event listeners that enable panning
			element.addEventListener("mouseleave", function () {overArea=false;})
	...
	var src = document.getElementById("srcFile");			//grab srcFile input and set value to zero so you can reload save file with same name.
	src.value= "";
}
